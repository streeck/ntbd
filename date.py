...
def createSQL():
    ...
    for single_date in daterange(start_date, end_date):
        if single_date.weekday() in [5, 6]:
            queries.append("INSERT INTO Tempo VALUES ({chave}, {diaNum}, '{diaSemana}', '{diaAbreviado}', {mesNum}, '{mesExtenso}', '{mesAbreviado}', {ano}, '{fds}')".format(chave=single_date.strftime("%Y%m%d"), diaNum=single_date.day, diaSemana=wkd[single_date.weekday()], diaAbreviado=wkdAbr[single_date.weekday()], mesNum=single_date.month, mesExtenso=mth[single_date.month-1], mesAbreviado=mthAbr[single_date.month-1], ano=single_date.year, fds='t'))
        else:
            queries.append("INSERT INTO Tempo VALUES ({chave}, {diaNum}, '{diaSemana}', '{diaAbreviado}', {mesNum}, '{mesExtenso}', '{mesAbreviado}', {ano}, '{fds}')".format(chave=single_date.strftime("%Y%m%d"), diaNum=single_date.day, diaSemana=wkd[single_date.weekday()], diaAbreviado=wkdAbr[single_date.weekday()], mesNum=single_date.month, mesExtenso=mth[single_date.month-1], mesAbreviado=mthAbr[single_date.month-1], ano=single_date.year, fds='f'))
    return queries
